...
class ETBUIndicatorsComputer3 extends ProjectIndicatorsComputer
{
    public function compute($request, $customAccess, $new, $datesOnlyQuery, $datesQuery)
    {
        ...
        $totalServicesNL = $this->getServicesCountByProfileField('6g6x', ['HeBk', 'dLBE'], [], $request, 'notin');
        ...
        foreach ($fullChildrenQuery->get() as $item) {
            ...
            if (!empty($item->profile[6][0]->data['GWQS'])
                ...
                if (count($soc3) || count($psy3)) {
                    ...
                    $fullChildren11Services1 = $fullChildren11Services1->concat($psy3ids1)->concat($soc3ids1);
                    $fullChildren11Services2 = $fullChildren11Services2->concat($psy3ids2)->concat($soc3ids2);
            ...
            if ((!$request->till || $request->till >= $item->profile[6][0]->data['kB3w'])
                ...
                if (count($soc4) || count($psy4)) {
                    ...
                    $fullChildren12Services1 = $fullChildren12Services1->concat($psy4ids1)->concat($soc4ids1);
                    $fullChildren12Services2 = $fullChildren12Services2->concat($psy4ids2)->concat($soc4ids2);
        ...
        foreach ($outcomesQuery->get() as $client) {
            if (!empty($client->profile['11']) && !empty($client->profile['11'][0]->data['TNF5'])
                ...
                if ($latestOutcome == 'bJjG' || $latestOutcome == 'PnKg') {
                    $outcomesTreated++;
                }

                if ($latestOutcome == 'nXDJ' || ($latestOutcome == 'A4Z9' && $latestReason)) {
                    $outcomesRecovered++;
                }

                if ($latestOutcome == 'dAtv' || ($latestOutcome == 'A4Z9' && !empty($latestData['BPcC']))) {
                    $outcomesLost++;
                }

                if ($latestOutcome == 'meHa') {
                ...
                if ($latestOutcome == 'EMsf') {
                    $outcomesBecameMDR++;
                }

                if ($latestOutcome == 'Ajf5') {
                    $outcomesBecameWDR++;
                }
            }
        }
        ...
        foreach ($outcomesQuery->get() as $client) {
            if (!empty($client->profile['11']) && !empty($client->profile['11'][0]->data['TNF5'])
                ...
                if ($latestOutcome == 'A4Z9' && $latestReason) {
        ...
        $vstHasReference = $vstHasReferenceQuery->count(DB::raw('distinct users.id'));
        ...
        static::addOutreachServicesClause($servicesOutreachNLQuery, $request);
    ...
    public static function addOutreachClause(
        ...
        $query->where(function ($query) use ($request, $parts, $position, $positionNe) {
            ...
            ->orWhere(function ($query) use ($request, $parts) {
                $query->where(function ($query) {
                    $query->whereNull('d.data->2CLW')
                        ->orWhere('d.data->2CLW', '!=', 'wx6s');
                })
                ...
                ->whereHas('activities', function ($query) use ($request, $parts) {
                    $query->where('project_id', 6);
